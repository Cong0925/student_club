<template>
    <div>
        <div class="activeCenter" v-if="!state.err">
            <div class="active-main" v-loading="state.loading">
                <div class="active-main-item" v-for="(item, index) in state.actives" :key="index">
                    <div class="active-main-item-left"
                        @click="$router.push({ name: 'ClubActiveDetails', params: { clubId: $router.currentRoute.value.params.clubId, activityId: item.activity_id } })"
                        style="cursor: pointer;">标题：{{ item.title }}</div>
                    <div class="active-main-item-left"
                        @click="$router.push({ name: 'ClubActiveDetails', params: { clubId: $router.currentRoute.value.params.clubId, activityId: item.activity_id } })"
                        style="cursor: pointer;">时间：{{ item.start_time }}</div>
                    <div class="active-main-item-left"
                        @click="$router.push({ name: 'ClubActiveDetails', params: { clubId: $router.currentRoute.value.params.clubId, activityId: item.activity_id } })"
                        style="cursor: pointer;">地址：{{ item.address }}</div>
...
import { toast, getCookies, setCookies } from "@/utils/tool"
